...
module Main
  def self.incorrect_input(symbol, choices, opponent)
    puts 'Incorrect input'
    input(symbol, choices, opponent)
  end

  def self.input(symbol, choices, opponent)
  ...
  def self.exitter(symbol)
    p "#{symbol} wins!"
    replay
  end

  def self.opponent_dialogue
  ...
  def self.game_start_board_display
    puts "
...
7 8 9"
  ...
  def self.game_play_board_display(board)
    puts "
  ...
  def self.run_game(player1, player2, game)
